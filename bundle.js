...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_1__src_local__ = __webpack_require__(203);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_3__src_mouse__ = __webpack_require__(204);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_7__src_select__ = __webpack_require__(205);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_8__src_selectAll__ = __webpack_require__(233);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_13__src_touch__ = __webpack_require__(234);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_14__src_touches__ = __webpack_require__(235);
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_26__datum__ = __webpack_require__(231);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_27__on__ = __webpack_require__(55);
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_28__dispatch__ = __webpack_require__(232);
...
exports.fetchPopulationByCountryYear = undefined;

var _parse_census_data = __webpack_require__(91);

var _parse_census_data2 = _interopRequireDefault(_parse_census_data);

var _selectors = __webpack_require__(92);

function _interopRequireDefault(obj) { return obj && obj.__esModule ? obj : { default: obj }; }

var fetchPopulationByCountryYear = exports.fetchPopulationByCountryYear = function fetchPopulationByCountryYear(country, year) {
  return $.ajax({
...
var fetchArraybyCountry = function fetchArraybyCountry(country) {

  for (var year = 2000; year <= 2010; year++) {
    fetchPopulationByCountryYear(country, year).then(function (res) {
      if (res && res[1][2]) {
        var yearR = res[1][45];
        var countryR = res[1][0];
        var parsed = (0, _parse_census_data2.default)(res);
        var males = (0, _selectors.extractGender)(parsed, 'M');
        var females = (0, _selectors.extractGender)(parsed, 'F');
...
var convertArrToPojo = function convertArrToPojo(arr) {
  var pojo = {};

  for (var i = 0; i < arr[0].length; i++) {
    if (arr[0][i].slice(1, 4) === 'POP' || arr[0][i] === 'POP') {
...
var addTotalPopAge = function addTotalPopAge(pojo) {
  var keys = Object.keys(pojo);
  var resPojo = Object.assign(pojo);

  for (var i = 0; i < keys.length; i++) {
    if (keys[i].slice(1, 4) === 'POP') {
      if (keys[i][0] === 'F') {
        var postfix = keys[i].slice(1);
        var maleNum = resPojo['M' + keys[i].slice(1)];
        var femaleNum = resPojo[keys[i]];
...
var calcPercent = function calcPercent(pojo) {
  var keys = Object.keys(pojo);
  var resPojo = Object.assign(pojo);

  for (var i = 0; i < keys.length; i++) {
    if (keys[i].slice(1, 4) === 'POP') {
      resPojo[keys[i] + 'p'] = parseFloat((resPojo[keys[i]] / resPojo['POP'] * 100).toFixed(3));
    }
  }
...
var group85plus = function group85plus(pojo) {
  var keys = Object.keys(pojo);
  var resPojo = Object.assign(pojo);

  var FPOP85_ = resPojo['FPOP85_89'] + resPojo['FPOP90_94'] + resPojo['FPOP95_99'] + resPojo['FPOP100_'];

  var MPOP85_ = resPojo['MPOP85_89'] + resPojo['MPOP90_94'] + resPojo['MPOP95_99'] + resPojo['MPOP100_'];

  var TPOP85_ = FPOP85_ + MPOP85_;

  var FPOP85_p = parseFloat((resPojo['FPOP85_89p'] + resPojo['FPOP90_94p'] + resPojo['FPOP95_99p'] + resPojo['FPOP100_p']).toFixed(3));

  var MPOP85_p = parseFloat((resPojo['MPOP85_89p'] + resPojo['MPOP90_94p'] + resPojo['MPOP95_99p'] + resPojo['MPOP100_p']).toFixed(3));

  var TPOP85_p = FPOP85_p + MPOP85_p;
  ...
  var toDelete = ['FPOP85_89', 'FPOP90_94', 'FPOP95_99', 'FPOP100_', 'MPOP85_89', 'MPOP90_94', 'MPOP95_99', 'MPOP100_', 'FPOP85_89p', 'FPOP90_94p', 'FPOP95_99p', 'FPOP100_p', 'MPOP85_89p', 'MPOP90_94p', 'MPOP95_99p', 'MPOP100_p', 'TPOP85_89', 'TPOP90_94', 'TPOP95_99', 'TPOP100_', 'TPOP85_89p', 'TPOP90_94p', 'TPOP95_99p', 'TPOP100_p'];
...
exports.default = function (arr) {
  return group85plus(calcPercent(addTotalPopAge(convertArrToPojo(arr))));
};
  ...
  for (var i = 0; i < keys.length; i++) {
    if (keys[i].slice(0, 4) === mf + 'POP') {
      if (keys[i][keys[i].length - 1] !== 'p') {
        var idx = indecing.indexOf(keys[i]);
        ...
        var tpop = 'TPOP' + keys[i].slice(4);
        resArr[idx]['numBoth'] = pojo[tpop];
        resArr[idx]['pctBoth'] = pojo[tpop + 'p'];
      } else {
        var _idx = indecing.indexOf(keys[i].slice(0, keys[i].length - 1));
        resArr[_idx]['pct'] = pojo[keys[i]];
      }
    }
  }
  return resArr;
...
var _d = __webpack_require__(94);

var d3 = _interopRequireWildcard(_d);

var _d3Scale = __webpack_require__(153);

var scale = _interopRequireWildcard(_d3Scale);
...
var x = scale.scaleLinear().range([0, width]);

var yBand = scale.scaleBand().domain(["", "85+", "80-84", "75-79", "70-74", "65-69", "60-64", "55-59", "50-54", "45-49", "40-44", "35-39", "30-34", "25-29", "20-24", "15-19", "10-14", "5-9", "0-4"]).rangeRound([0, 380]);

window.yBand = yBand;

var yAxis = d3.axisLeft().scale(yBand);

var generateXaxis = function generateXaxis(sc) {
  return d3.axisBottom().scale(sc).tickValues([0, 2, 4, 6, 8, 10]).tickFormat(function (d) {
    return d + "%";
  });
};
...
document.addEventListener("DOMContentLoaded", function () {
  var mainChart = d3.select('.chart').attr("width", mainWidth + margin.left + margin.right).attr("height", mainHeight + margin.top + margin.bottom);

  mainChart.append("g").attr("class", "x axis").attr("transform", "translate(97," + (margin.top + 360) + ")").call(generateXaxis(x.domain([12, 0])));

  mainChart.append("g").attr("class", "x axis").attr("transform", "translate(349," + (margin.top + 360) + ")").call(generateXaxis(x.domain([0, 12])));

  mainChart.append("g").attr("class", "y axis").attr("transform", "translate(97," + (margin.top - 10) + ")").call(yAxis);

  mainChart.append("text").attr("class", "gender-labels").attr("x", "250").attr("y", "" + margin.top).text("MALE");

  mainChart.append("text").attr("class", "gender-labels").attr("x", "510").attr("y", "" + margin.top).text("FEMALE");

  mainChart.append("text").attr("class", "axis-labels").attr("x", "88").attr("y", "" + (margin.top - 1)).text("Age Group");

  mainChart.append("text").attr("class", "axis-labels").attr("x", "230").attr("y", "" + (margin.top + 390)).text("Percentage of total population");

  mainChart.append("text").attr("class", "tpop").attr("x", "310").attr("y", "" + (margin.top + 390));

  mainChart.append("text").attr("class", "source").attr("x", "600").attr("y", "" + (margin.top + 390)).text("Source: U.S. Census Bureau");
...
var drawChart = function drawChart(gender, data) {
  ...
  d3.select('.tpop').text("(" + numberWithCommas(data[0].tpop) + ")").style('font-weight', 'bold');

  var chart = d3.select("." + gender).attr("width", width).attr("height", barHeight * data.length);
  ...
  bar.merge(barEnter).transition().duration(1000).attr("transform", function (d, i) {
    return "translate(" + offset(d) + "," + (margin.top + i * barHeight) + ")";
  });

  bar.select('rect').merge(barEnter).attr("height", barHeight - 1).transition().duration(1000).attr("width", function (d) {
    return x(d.pct);
  });
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_22_d3_scale__ = __webpack_require__(153);
...
var _census_api_util = __webpack_require__(90);

var _agegroup = __webpack_require__(93);
...
exports.clickCallback = exports.addLinks = exports.addYear = exports.addCountryName = undefined;
...
var renderChart = function renderChart(_ref) {
  var year = _ref.year,
      country = _ref.country,
      parsed = _ref.parsed,
      males = _ref.males,
      females = _ref.females;

  addCountryName([country]);
  addYear([year]);
  (0, _agegroup2.default)('males', males);
  (0, _agegroup2.default)('females', females);
};

var addCountryName = exports.addCountryName = function addCountryName(data) {
  var div = d3.select('.chart-header').selectAll('div').data(data);

  var enterDiv = div.enter().append("div");

  div.merge(enterDiv).text(function (d) {
    return d;
  });
};

var addYear = exports.addYear = function addYear(data) {
  var div = d3.select('.chart-footer').selectAll('div').data(data);
...
var addLinks = exports.addLinks = function addLinks(data) {
  var div = d3.select('.fetch-links').selectAll('div').data(data);
...
var clickCallback = exports.clickCallback = function clickCallback(country, year) {
  return (0, _census_api_util.fetchPopulationByCountryYear)(country, year).then(renderChartAfterFetching);
};

var renderChartAfterFetching = function renderChartAfterFetching(res) {
  if (res && res[1][2]) {
